CatHorizontalBar = function(data, tox_include, cats = unique(data$CTC_CAT1)){
  ...
  if(type == "Percent"){
    ...
    summary_df = merge(summary_df, tot_num, by = "TRNO1")
    summary_df$N = summary_df$N/summary_df$tot*100
    summary_df=summary_df %>% select(CTC_CAT1, TRNO1, TOXDEG1, N, TOXLABEL1)
  }
  ...
  summary_df$TOXLABEL_TEMP = summary_df$CTC_CAT1
  summary_df$CTC_CAT1 = summary_df$TOXLABEL1
  summary_df$TOXLABEL1 = summary_df$TOXLABEL_TEMP
  summary_df = summary_df %>% select(-TOXLABEL_TEMP)
  ...
  p1 = p +
    ...
    geom_segment(data = base_df1, 
                 aes(x = -(start - 0.5), y = 0, 
                     xend = -(end + 0.5), yend = 0)) +
    scale_y_reverse(limits = c(max_trt_count + 1, 0)) +
  ...
  p2 = p +
    ...
    geom_segment(data = base_df2, 
                 aes(x = -(start - 0.5), y = 0, 
                     xend = -(end + 0.5), yend = 0)) +
    scale_y_continuous(limits = c(0, max_trt_count + 1))+
    theme(axis.text.x = element_text(colour = "grey60", size = 14),
          axis.text.y = element_blank(),
          #axis.title = element_blank(),
          panel.grid.major.y = element_blank(),
  ...
  if(type == "Percent"){
    p_label = ggplot() +
      ...
      geom_text(data = base_df1,
                aes(x = - title, y = 0, label = CTC_CAT1),
  ...
  p_sublabel = ggplot() +
    geom_text(data = label_df1, 
              aes(x = -id, y = 0, 
                  label = ifelse(is.na(id_label), 
                                 NA, 
                                 paste0(id_label, ":", TOXLABEL))),
              hjust = 0, size = 6) +
    coord_flip() +
    theme_minimal() +
    theme(axis.text = element_text(colour = "grey", size = 14),
          axis.title = element_blank(),
          plot.title = element_text(size = 25, 
                                    face = "bold",
                                    hjust = 0.5))+
